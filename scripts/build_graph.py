...
def build_edgelist(fp, c2i):
    ...
    with open(fp) as fh:
        for l in tqdm(fh):
            num_docs += 1
            doc = json.loads(l)
            n = min(len(doc['mesh']), len(doc['kw']))
            concepts = [c2i[c] for c in random.sample(doc['kw'], n) + \
                                        random.sample(doc['mesh'], n)
                        if c in c2i]
    ...
    # consideration: why do we choose NMI intead of just cooccurrences?
    # what's the benefit or effects of NMI in building a graph and
    # consequences.
...
if __name__ == '__main__':
    ...
    edge_metric = 'freq'    # nmi: normalized mutual information or 
                           # freq: cooccurrence frequency
    
    # Build a vocabulary of biomedical concepts (i.e., mesh and keywords)
    meshes = read_mesh(mesh_file)
    keywords = read_kw(data_json, n=len(meshes))
